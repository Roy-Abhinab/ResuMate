...
const Dashboard = () => {
   ...
   const createResume = async (event: React.SubmitEvent<HTMLFormElement>) => {
   ...
   const uploadResume = async (event: React.SubmitEvent<HTMLFormElement>) => {
      event.preventDefault()
      setIsLoading(true)

      try {
         const resumeText = await pdfToText(resume as File)
   ...
   return (
      <div>
         <div className='max-w-7xl mx-auto px-4 py-8'>
            ...
            {showCreateResume && (
               <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur z-10 flex items-center justify-center'>
                  <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                     ...
                     <input
                        ...
                        className='w-full px-4 py-2 mb-4 focus:border-green-600 outline-none ring-1 ring-slate-300 focus:ring-green-600 rounded'
                        required
                     />
            ...
            {editResumeId && (
               <form onSubmit={editTitle} onClick={() => setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur z-10 flex items-center justify-center'>
                  <div onClick={(e) => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
